...
def create_app(config=None):
    if config is None:
        logging.error('Please provide the config file. Exiting...')
        sys.exit(99)
    with open(config, 'r') as config_handle:
        cfg = yaml.load(config_handle)
    ...
    app.config['SQLALCHEMY_DATABASE_URI'] = "mysql://{username}:{password}@{host}/{db}".format(username=cfg['mysql']['user'],
